							...
							<s:form action="BudgetFormModif">
							  	...
							  	<div class="modal-body">
							  		<c:if test="${sessionScope.type == 'admin' || sessionScope.idU == b.getIdU()}">
										...
										<s:textfield label="Buget gaz" name="b.budgetGaz" value="%{#attr['b.getBudgetGaz()']}" required="required"/>
										<s:textfield label="Buget entretien" name="b.budgetEntretien" value="%{#attr['b.getBudgetEntretien()']}" required="required"/>
										<s:textarea label="Commentaire" name="b.commentaire" value="%{#attr['b.getCommentaire()']}" required="required" cols="20" rows="10"/>
									</c:if>
